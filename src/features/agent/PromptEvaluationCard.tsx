...
export function PromptEvaluationCard({ evaluation }: { evaluation: PromptEvaluation }) {
    ...
    >
      <div className="flex items-center justify-between border-b border-white/10 pb-3">
        <div className="flex items-center gap-2 text-indigo-300 font-medium">
          <Sparkles size={16} />
          <span>Prompt Mentor Analysis</span>
        </div>
      ...
      {evaluation.weaknesses.length > 0 && (
        <div className="space-y-2">
          <span className="text-xs uppercase tracking-wider text-white/50 font-semibold">Areas to Improve</span>
      ...
      <div className="bg-indigo-900/30 border border-indigo-500/20 rounded-lg p-3 space-y-2">
        ...
        <button className="w-full mt-2 flex items-center justify-center gap-2 text-xs bg-indigo-500/20 hover:bg-indigo-500/40 text-indigo-200 py-1.5 rounded transition-colors">
